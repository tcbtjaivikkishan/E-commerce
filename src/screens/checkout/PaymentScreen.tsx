...
import { clearCart, selectSubtotal, selectTotalItems } from '../../store/cartSlice';
...
export default function PaymentScreen() {
  const { tempPayment, updatePayment, submitOrder, isReadyToPlace } = useOrder();
  const subtotal   = useAppSelector(selectSubtotal);
  const totalItems = useAppSelector(selectTotalItems);

  const handlePlaceOrder = () => {
    if (isReadyToPlace && subtotal > 0 && totalItems > 0) {
      submitOrder(subtotal, totalItems);
      clearCart();
