...
const Dot = ({ x, y, size, delay }: { x: number; y: number; size: number; delay: number }) => {
  ...
  return (
    <Animated.View style={{
      ...
      width: size, height: size, borderRadius: size / 2, backgroundColor: '#bbf7d0',
...
export default function LoginScreen() {
  ...
  const handleLogin = () => {
    ...
    Animated.sequence([
      ...
      setTimeout(() => { setLoading(false); router.replace('/'); }, 1400);
    });
  };
  ...
  return (
    <KeyboardAvoidingView style={S.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <View style={S.wave}>
        ...
        <Dot x={300} y={50} size={7} delay={700}/>
        <Dot x={160} y={100} size={5} delay={400}/>
        <Dot x={60} y={120} size={8} delay={1100}/>
      </View>
      ...
      <ScrollView contentContainerStyle={S.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={S.logoSection}>
          ...
          <Text style={S.sub}>Fresh · Organic · Pure</Text>
        </View>

        {/* ── Card ── */}
        <Animated.View style={[S.card, { opacity: cardOpacity, transform: [{ translateY: cardY }] }]}>
          <View style={S.cardBar}/>
          <Text style={S.cardHead}>Sign In</Text>
          <Text style={S.cardCaption}>Welcome back! Continue your healthy journey.</Text>

          {/* Email */}
          <View style={S.fg}>
            <Text style={S.lbl}>Email or Phone</Text>
          ...
          <Animated.View style={{ transform: [{ scale: btnScale }] }}>
            <TouchableOpacity style={S.cta} onPress={handleLogin} activeOpacity={0.88}>
              {loading ? <LoadingDots/> : <Text style={S.ctaTxt}>Sign In</Text>}
            </TouchableOpacity>
          </Animated.View>

          {/* Divider */}
          <View style={S.div}>
            <View style={S.divL}/><Text style={S.divT}>or continue with</Text><View style={S.divL}/>
          </View>
          ...
          {/* Switch */}
          <View style={S.sw}>
            <Text style={S.swT}>Don't have an account? </Text>
            <TouchableOpacity onPress={() => router.push('/signup')}>
              <Text style={S.swL}>Create one</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
        ...
        {/* Trust */}
        <View style={S.trust}>
          {['🔒 Secure Checkout', '🌿 Certified Organic', '🚚 Fast Delivery'].map(t => (
...
const S = StyleSheet.create({
  ...
  wave: {
    ...
    backgroundColor: '#16a34a', borderBottomLeftRadius: 56, borderBottomRightRadius: 56,
    overflow: 'hidden',
  },
  scroll: { flexGrow: 1, paddingHorizontal: 20, paddingBottom: 36 },
  logoSection: { alignItems: 'center', paddingTop: 56, paddingBottom: 24 },
  logoBox: {
    width: 66, height: 66, borderRadius: 22, backgroundColor: '#fff',
    alignItems: 'center', justifyContent: 'center', marginBottom: 14,
    shadowColor: '#166534', shadowOpacity: 0.25, shadowRadius: 14, shadowOffset: { width: 0, height: 4 }, elevation: 8,
  },
  brand: { fontSize: 26, fontWeight: '800', color: '#fff', letterSpacing: 0.4 },
  sub: { fontSize: 12, color: '#dcfce7', marginTop: 3, letterSpacing: 1.2, opacity: 0.9 },

  card: {
    backgroundColor: '#fff', borderRadius: 28, padding: 26, marginTop: 4,
    shadowColor: '#166534', shadowOpacity: 0.1, shadowRadius: 28, shadowOffset: { width: 0, height: 8 }, elevation: 10,
    overflow: 'hidden',
  },
  cardBar: { position: 'absolute', top: 0, left: 0, right: 0, height: 4, backgroundColor: '#16a34a' },
  cardHead: { fontSize: 23, fontWeight: '800', color: '#111827', marginTop: 6, marginBottom: 5 },
  cardCaption: { fontSize: 13, color: '#9ca3af', marginBottom: 22, lineHeight: 19 },
  ...
  inp: {
    ...
    borderRadius: 14, paddingHorizontal: 15, height: 52,
  },
  ...
  forgotTxt: { color: '#16a34a', fontSize: 12, fontWeight: '700' },

  cta: {
    backgroundColor: '#16a34a', borderRadius: 16, height: 54,
    alignItems: 'center', justifyContent: 'center',
    shadowColor: '#16a34a', shadowOpacity: 0.4, shadowRadius: 16, shadowOffset: { width: 0, height: 6 }, elevation: 8,
  },
  ctaTxt: { color: '#fff', fontSize: 16, fontWeight: '800', letterSpacing: 0.4 },

  div: { flexDirection: 'row', alignItems: 'center', gap: 10, marginVertical: 20 },
  divL: { flex: 1, height: 1, backgroundColor: '#f3f4f6' },
  divT: { color: '#d1d5db', fontSize: 12, fontWeight: '500' },

  socials: { flexDirection: 'row', gap: 12, marginBottom: 20 },
  socBtn: {
  ...
  trust: { marginTop: 24, gap: 6, alignItems: 'center' },
